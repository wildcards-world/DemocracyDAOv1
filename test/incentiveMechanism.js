const {
  ...
  contract("noLossDao", (accounts) => {
    ...
    it("NoLossDao:incentiveMechanism. Interest is split correctly to the user if they vote consistantly", async () => {
      ...
      assert.equal(voter1BalanceBeforeVote.toString(), "0", "balance should be zero at the beginning of the period");
      // Because they voted in the previous iteration they are eligible to recieve a cut of the interest earned.
      const {voteIncentiveStake: voteIncentiveStakeVoter1} = await poolDeposits.usersVotingCreditAndVoteIncentiveState.call(
        voter1
      );
      const {voteIncentiveStake: voteIncentiveStakeVoter2} = await poolDeposits.usersVotingCreditAndVoteIncentiveState.call(
        voter2
      );
      assert.equal(voteIncentiveStakeVoter1.toString(), mintAmount1.sub(new BN(_daoMembershipMinimum).mul(new BN(80)).div(new BN(100))).toString(), "Voters stake is incorrect");
      const totalStakeVotedInPreviousIteration = await noLossDao.totalUsersVotesInIteration.call(proposalIterationOfFirstVote);
      assert.equal(voteIncentiveStakeVoter1.add(voteIncentiveStakeVoter2).toString(), totalStakeVotedInPreviousIteration.toString(), "TotalStaked isn't correct");
      const totalAvailablePayoutPreviousIteration = await noLossDao.totalIterationVotePayout.call(proposalIterationOfFirstVote);
      assert.equal(totalInterestEarnedForFirstVote.toString(), totalAvailablePayoutPreviousIteration.toString(), "TotalStaked isn't correct");
      ...
      assert.equal(
        ...
        totalInterestEarnedForFirstVote.mul(voteIncentiveStakeVoter1).div(totalStakeVotedInPreviousIteration).toString(),
