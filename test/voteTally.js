...
contract("noLossDao", (accounts) => {
  ...
  it("noLossDao:voteTally. Votes tally gets correctly registered.", async () => {
    ...
    await noLossDao.voteDirect(proposalID1, 16, 4, { from: accounts[1] });
    await noLossDao.voteDirect(proposalID2, 25, 5, { from: accounts[2] }); // this proposal should win
    ...
    assert.equal("4", votesForProposal1.toString());
    assert.equal("5", votesForProposal2.toString());
    ...
    await noLossDao.voteDirect(proposalID1, 36, 6, { from: accounts[1] }); // 1 gets 6 votes
    await noLossDao.voteDirect(proposalID3, 4, 2, { from: accounts[1] }); // 3 gets 2 votes

    await noLossDao.voteDirect(proposalID1, 49, 7, { from: accounts[2] }); // 1 gets 7 votes
    await noLossDao.voteDirect(proposalID3, 49, 7, { from: accounts[2] }); // 3 gets 7 votes

    await noLossDao.voteDirect(proposalID1, 36, 6, { from: accounts[5] }); // 1 gets another 6 votes
    await noLossDao.voteDirect(proposalID3, 64, 8, { from: accounts[5] }); // 3 gets another 6 votes
    ...
    assert.equal(votesForProposal1NextIteration.toString(), "19");
    assert.equal(votesForProposal2NextIteration.toString(), "17");
